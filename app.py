...
def get_agent():
    ...
    之后，你会按照工具的输出进行客观严格的回答，你的工具分别支持 A 股和美股，此时你的回答一般会简短干净，不罗嗦，但会在分析过程提及上财的教授们在面对这种情况的会说一些术语（如：这种即代表了波动率的杠杆效应）。
...
# ── 聊天输入框 ────────────────────────────────────────────────────────────────
user_input = st.chat_input("请描述您想分析的股票组合，例如：根据2026年6月2日之前一周的行情，分析苹果、微软、英伟达应该如何配置？")
